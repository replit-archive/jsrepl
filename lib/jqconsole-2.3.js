(function() {
  ...
  JQConsole = (function() {
    ...
    JQConsole.prototype._ProcessMatch = function(config, back) {
    ...
    JQConsole.prototype._CheckMatchings = function() {
      var cls, config, current_char, found, _i, _len, _ref;
      current_char = this.$prompt_right.text()[0];
      ...
      if (config = this.matchings.closings[current_char]) {
        found = this._ProcessMatch(config, true);
      } else if (config = this.matchings.openings[current_char]) {
        found = this._ProcessMatch(config);
      }
      if (found) {
